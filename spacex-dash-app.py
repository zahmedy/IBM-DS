...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown', options=[
                                    ...
                                    value='ALL',
                                    placeholder="Select a luanch site",
                                    searchable=True),
                                html.Br(),
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site', 
        title='Launch Success Rate per Site')
        return fig
    else:
        succes_n_fail = filtered_df.groupby(['Launch Site', 'class']).size().reset_index(name='count')
        fig = px.pie(succes_n_fail, 
        values='count', 
        names='class', 
...
def get_scatter_chart(entered_site, entered_payload):
    ...
    fig = px.scatter(
        ...
        title=f'Success Rate for {entered_site}' if entered_site != 'ALL' else 'Success Rate for All Launch Sites'
    )
    return fig
